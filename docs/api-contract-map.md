...
| Auth | POST | `/auth/login-face` | `auth.service.ts` | `FaceLoginModal` | `{ faceImageBase64 }` | `{ token }` | Aderente |
...
| Documents | DELETE | `/documents/:documentId` | `document.service.ts` | `Documentos`, `useEmployeeDocuments`, `useMyDocuments` | sem payload | `204/200` | Aderente |
